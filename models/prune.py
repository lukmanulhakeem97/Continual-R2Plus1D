...
class Pruner(object):
    ...
    def _pruning_mask(self, weights, previous_mask, layer_idx):
        ...
        mask = previous_mask
        used_perc = 100 * mask.eq(0).sum() / tensor.numel()
        print(
            "Layer #%d, pruned %d/%d (%.2f%%) (Total in layer: %d)"
            % (layer_idx, mask.eq(0).sum(), tensor.numel(), used_perc, weights.numel())
        )
        return mask, used_perc

    def is_network_exhausted(self, prun_perc):
        condition = any(map(lambda x: x > 99., prun_perc.values()))
        if condition:
            self.shift_mode = True
            print("Network is being exhausted!")
    ...
    def prune(self):
        ...
        for module_idx, (name, module) in enumerate(self.model.named_modules()):
            ...
            if isinstance(module, nn.Conv3d) or isinstance(module, nn.Linear):
                mask, used_perc = self._pruning_mask(module.weight.data, self.previous_masks[module_idx], module_idx)
                # self.current_masks[module_idx] = mask.cuda()
                self.network_usage[module_idx] = used_perc
                self.current_masks[module_idx] = mask.to(self.device)
                # Set pruned weights to 0.
                weight = module.weight.data
                weight[self.current_masks[module_idx].eq(0)] = 0.0
        self.is_network_exhausted(self.network_usage)
    ...
    def train_epoch(self, dataloader, loss_fn, device, task_nr, verbose=True):
        ...
        self.model.set_bn_layer(task_nr)
